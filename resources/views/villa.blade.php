@extends('partial.layout-detail')

{{--*/ $title = $villa[0]->title. ' | '.$area[0]->title. ' villa rentals'; /*--}}
@section('title', $title)
@section('content')
<!-- villa-detail -->
			<div class="villa-detail col-sm-12" ng-app="frontend">
				...
				<div class="modal-body">
					<div class="col-xs-12">
					<form class="form-horizontal" action="{{ url('detail/order') }}" method="post">
						<?php echo Form::token(); ?>
						<input type="hidden" name="villa-id" value="{{ $villa[0]->id }}"/>
				...
				<form class="form-horizontal" action="{{ url('detail/order') }}" method="post">
					...
					<div class="arrow-down"><img src="{{ URL::asset('assets/images/icon-arrow-down.png')}}"></div>
					<div id="form-contact-wrapper">
							<div class="form-group">
								<div class="row">
									<label style="width:80px;">Checkin: </label>
							...
							<div class="form-group verification-code captcha-container" style="margin:0 auto !important;">
								<div class="g-recaptcha" data-sitekey="{{ env('RE_CAP_SITE') }}" data-callback="capenable" data-expired-callback="capdisable"></div>
							</div>
							<button type="submit" class="btn btn-default btn-submit" style="margin:0px 10%; width:80%;">Order Now</button>
					</div>	
				</form>
				...
				<div id="detail-villa" class="col-sm-9" ng-controller="homeController">
					<h1> {{ strtoupper($villa[0]->title) }} </h1>
					@if (isset($villa[0]->intro))
						{!! $villa[0]->intro !!} <a href="#">More Detail</a>
					@else
						{!! str_limit($villa[0]->description, 300) !!} <a href="#">More Detail</a>
					@endif
				        ...
				        <div class="resp-tabs-container hor_1">
									...
									<h2> Gallery </h2>
										<div class="gallery-image">
										   <span ng-repeat="gallery in galleryByVilla">
											<a href="@{{ gallery.url }}" data-lightbox="@{{ gallery.name }}" data-title="@{{ gallery.caption }}"><img src="{{ URL::asset('assets/images/lazy-image-detail.png')}}" lazy-img="@{{ gallery.thumbUrl }}" /></a>
										   </span>
										</div>
								</div>
				            </div>
				            	...
				            	<div class="rates-detail">
					            	...
					            	<div class="table-responsive">
										...
										<table class="table">
											...
											<tr> 
												<td width="50%"> {{ $rate->season }}</td>
												<td>{{ isset($rate->min_stay) ? $rate->min_stay . ' Nights' : '' }}</td>
												<td>USD {{ $rate->rate }} {{ $rate->plus === 1 ? '++' : '' }} {{ isset($rate->tax) ? 'Tax: '.str_replace('.00', '%', $rate->tax) : '' }}</td>
											</tr>
											@endforeach
										</table>
										@endforeach
							...
							<div id="services" class="section">
								...
								<div class="services-detail">
					            	@if (!empty($villa[0]->services) || !empty($villa[0]->facilities))
									 {!! $villa[0]->services !!}
									 {!! $villa[0]->facilities !!}
									 @endif
				            	</div>
							...
							<div id="staff" class="section">
								<h2>Staff Detail</h2>
								<div class="services-detail">
									@if (!empty($villa[0]->staff_detail))
									 {!! $villa[0]->staff_detail !!}
									 @endif
								</div>
							</div>
...
@endsection
@section('mapjs')
<script src="{{ URL::asset('assets/js/jquery.throttle.js')}}"></script>
<script type="text/javascript">
    ...
    var locations = [
      ['{{ $villa[0]->title }}', {{ $villa[0]->latitude }}, {{ $villa[0]->longitude }}],
    ];
